# ```Здесь должны появиться описания классов для v2```




# ```Функции обрабатывающие только список документов```

def person_search(documents_list):
...
# ```Функции обрабатывающие только список полок```

def shelf_search(directories_list):
    doc_num = input('Введите номер документа > ')
...
# ```Функции обрабатывающие оба списка```
...
# ```Функции чтения\записи данных в файлы```
...
# Функция пользовательского управления
#
def main(doc_file=None, dir_file=None):
    help = {
      ...
      's': ['save', 'Сохранить файлы / Данные в новый файл'],
    ...
    if doc_file == None:
        doc_file = './saved/init_docs.txt'
    if dir_file == None:
        dir_file = './saved/init_dirs.txt'
    ...
    while user_choise != "e":
        ...
        elif user_choise == 'save':
            print('\n# Сохранение копии в папке ./saved/')
            bkp_name = input('Введите префикс имени файлов> ')
            doc_file_bkp = './saved/' + str(bkp_name) + '_docs.txt'
            dir_file_bkp = './saved/' + str(bkp_name) + '_dirs.txt'
            save_to_files(documents_list, doc_file_bkp, directories_list, dir_file_bkp)
        elif user_choise == 'h':
...
main('current_docs.txt', 'current_dirs.txt')
